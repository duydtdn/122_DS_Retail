...
const editCart = (cartId) => {
  window.location.assign(`/detail.html?cart=${cartId}`)
}
...
const renderCart = () => {
  ...
  const stringHtml =
    cartItems.map(it =>
      ...
      <div class="info">
        ...
        <div class="total">Total: ${+it.price.slice(0, -1) * it.quantity}$</div>
      </div>
      <div class="action">
  ...
  <div class="total-cart">
    <span>Total:</span>
    <span>${cartItems.map(c => +c.price.slice(0, -1) * c.quantity).reduce((a, b) => a + b, 0)}$</span>
  </div>`
